...
namespace MaHoaDES.DoiTuong
{
    class DES64Bit
    {
        ...
        /// gồm 1 khóa để mã hóa các chuỗi vào
        /// </summary>
        public Khoa KhoaDES { get; private set; }
        public ChuoiNhiPhan ThucHienDES(Khoa key,ChuoiNhiPhan ChuoiVaoDai, int MaHoaHayGiaiMa)// 1 ma hoá, -1 giải mã
        {
            ...
            KhoaDES.SinhKhoaCon(); // sinh dãy các khóa con
            ...
            for (int k = 0; k < DSChuoiVao.Length; k++)  // duyêt qua từng chuỗi được
            {
                ...
                for (int i = 0; i < 16; i++)
                {
                    // tính hàm F giữa khóa con và Right
        ...
        public string ThucHienDESText(Khoa key,string ChuoiVao, int MaHoaHayGiaiMa)// 1 ma hoa, -1 giai ma
        {
            ...
            return ChuoiNhiPhan.ChuyenNhiPhanSangChu(KQ);// chuyển sang dạng text để hiện thị kết quả
        }       
        /// <summary>
        /// Hàm tính F 
        /// đầu vào: chuỗi cần tính (C0 hoặc D0) và khóa con thứ k
        /// </summary>
        /// <param name="chuoiVao"></param>
        ...
        private ChuoiNhiPhan HamF(ChuoiNhiPhan chuoiVao, ChuoiNhiPhan KhoaCon)
        {
            ...
            KQ = KQ.XOR(KhoaCon); // tính XOR giữa kq hiện tại và khóa con 
            KQ = CacThongSo.TinhSBox(KQ); // tính hộp s-box
            KQ = CacThongSo.TinhP(KQ); // tính hoán vị P
            return KQ;
        }
